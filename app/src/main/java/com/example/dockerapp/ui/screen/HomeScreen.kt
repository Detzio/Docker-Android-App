...
fun ContainerCard(
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            text = { 
                Text("Êtes-vous sûr de vouloir supprimer le conteneur '${container.names?.firstOrNull()?.removePrefix("/") ?: container.id.take(12)}' ?")
            },
            confirmButton = {
                TextButton(
                    onClick = {
                        homeViewModel.deleteContainer(container.id)
